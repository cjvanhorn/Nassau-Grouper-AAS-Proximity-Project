...
# NG == Nassau Grouper
# CAS == Courtship-associated sounds
# DAFS == Days After First Spawn
# ST == SoundTrap hydrophone
# LARS == DSG-LARS hydrophone
...
data_list <- list(
   N = nrow(data), # cumulative hours observed for all hydrophones
   tothrs = length(unique(data$hour.of.period)), # total hours monitored during
                                                 # the study period
   hr = as.integer(data$hour.of.period), # hour of study period (1 - 158)
   hydro = as.integer(as.factor(data$Hydro)), # 1 = LARS1, 2 = ST2, etc.
                                              # Note there is no 3 because
                                              # LARS3 malfunctioned and did not
                                              # record any data
   cas = data$cas.count, # counts of CAS 
   m = data$minutes.observed / 60, # minutes observed per hour conversion
   timeofday = as.integer(data$Hour + 1), # add 1 to allow logs of hour 0
                                          # (12:00AM == 1 ... 11:00PM == 24)
   DayOfSeason = as.integer(as.factor(data$DAFS)), # Days After First Spawn
                                                   # (4, 3, 2, 1 before spawn,
                                                   # 5, 6, 7 after spawn)
   fishProx = as.integer(as.factor(data$fish.prox)) # 1 = NG not observed, 
                                                    # 2 = NG nearby (20-100m), 
                                                    # 3 = NG present (0-20m)
)
...
set.seed(234) # set seed for duplication
n_iter = 1000 # number of iterations per chain
n_chains = 3 # number of chains in MCMC
...
mod <- { ulam(
   ...
   iter = n_iter, control = list(max_treedepth = 15, adapt_delta = 0.95)) }
...
points(2, 8, pch = 19, cex = 5, col = 'Yellow')
points(2, 8, cex = 5)
